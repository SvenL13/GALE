...
From repo: https://github.com/canbooo/duqo

https://github.com/canbooo/duqo/blob/master/duqo/doe/lhs.py

DoE - LHS and orthogonal sampling
"""
import warnings
...
import numpy as np
from scipy.linalg import cholesky, eigh, inv, pinv
...
def uniform_lhs(lower_bounds, upper_bounds, num_sample, corr_mat=0, num_iter=100):
    """
    ...
    if np.isscalar(corr_mat):
        corr_mat = (
            np.eye(num_var) * (1 - corr_mat) + np.ones((num_var, num_var)) * corr_mat
        )
...
def _switch_rows(doe_curr: np.ndarray, column: int = None, col_row_pairs=()):
    """
    ...
    doe_curr[row_1, cur_column], doe_curr[row_2, cur_column] = (
        doe_curr[row_2, cur_column],
        doe_curr[row_1, cur_column],
    )
...
def _calc_score(doe_curr_, corr_mat_, dist_max_, appender_, appender_loc_):
    dist_score = _calc_distscore(appender_(doe_curr_), dist_max_)
    corr_score = _calc_corrscore(appender_loc_(doe_curr_), corr_mat_)
    return 5 * dist_score + corr_score


def optimize_doe(
    doe_start: np.ndarray,
    corr_mat: Union[float, np.ndarray] = 0,
    doe_old: Optional[np.ndarray] = None,
    num_tries: int = 10000,
    decay: float = 0.95,
    max_steps: int = 20,
    sim_time: float = 25.0,
    ...
    Inputs
    ------
    doe_start : np.ndarray, shape=(n_samples, n_features)
        used as the started solution
    corr_mat : float or np.ndarray
        ...
        no correlation.
    doe_old: np.ndarray, shape=(n_samples, n_features), optional (default=None)
        samples to preserve
    num_tries : int
    ...
    Returns
    -------
    doe_final : np.ndarray, shape=(n_samples, n_features)
        Optimized design of experiments matrix
    """
    num_var: int = doe_start.shape[1]
    ...
    if doe_old is None:
        ...
        locs = np.logical_and((doe_old >= locs[0]).all(1), (doe_old <= locs[1]).all(1))

        appender_loc = lambda x: np.append(
            doe_old[locs].reshape((-1, x.shape[1])), x, axis=0
        )
        appender = lambda x: np.append(
            doe_old, x, axis=0
        )  # will be used for calculating score
    ...
    best_score = _calc_score(doe_final, corr_mat, dist_max, appender, appender_loc)
    ...
    if verbose > 0:
        ...
        print(
            f"Start loss - total: {best_score:.4f} dist: {5 * dist_score:.4f} corr: {corr_score:.4f}"
        )
    ...
    for i_try in range(num_tries):
        ...
        curr_score = _calc_score(doe_try, corr_mat, dist_max, appender, appender_loc)
        anneal_prob = 0.0
        ...
        if (
            curr_score <= start_score or np.random.random() <= anneal_prob
            ...
            if start_score < best_score:
                ...
                if verbose > 1:
                    ...
                    corr_score = _calc_corrscore(doe_final, corr_mat)
                    print(
                        f"{i_try + 1} - total: {best_score:.4f} dist: {5 * dist_score:.4f} corr: {corr_score:.4f}"
                    )
    ...
    if verbose > 0:
        ...
        print(
            f"Final loss - total: {best_score:.4f} dist: {5 * dist_score:.4f} corr: {corr_score:.4f}"
        )
...
def inherit_lhs(
    num_sample: int, empty_bins: np.ndarray, bounds_l: np.ndarray, bounds_u: np.ndarray
) -> np.ndarray:
    """
    ...
    ) / num_bins  # get bin mids i.e. 0.5/num_sample, 1.5/num_sample...
    ...
    for i_dim in range(num_dims):
        ...
        while n_diff != 0:
            ...
            if n_diff > 0:
                ...
                cur_bins = np.array(
                    [c for i, c in enumerate(cur_bins) if i not in extras]
                )
                n_diff = 0
...
def make_doe(
    ...
    """
    if margs is None:
        margs = [uniform(lb, ub - lb) for lb, ub in zip(lower_bound, upper_bound)]
    if num_sample == 1:
    ...
    if np.isscalar(corr_mat):
        corr_mat = (
            np.eye(num_dims) * (1 - corr_mat) + np.ones((num_dims, num_dims)) * corr_mat
        )
    n_iter = num_tries // 5
...
def find_empty_bins(
    doe: np.ndarray, n_bins: int, lower_bound: np.ndarray, upper_bound: np.ndarray
) -> np.ndarray:
    """
    ...
    for i_bin in range(n_bins):
        condition = np.logical_and(probs >= edges[i_bin], probs <= edges[i_bin + 1])
